...
Calculate MD5 sum
    ...
    Input:  src -- filename or string
            type -- 0=file, 1=string
    Output: sum
...
int md5_sum(char *src, char *sum, int type)
{
    ...
    if (!type) { //file
        ...
        md5_write (&ctx, (unsigned char*)src, strlen(src));
    }

    md5_final(&ctx);
    for (i = 0; i < 16; i++)
        sprintf (&sum[2 * i], "%02x", ctx.buf[i]);
    return 0;
}

/*!
Read MD5 sums from the FILE and check them

    Input:  md5file -- file that contain MD5 sums
    Return: 0=success, >0=failure
*/
int md5_check(char *md5_file)
{
    FILE *fp = fopen (md5_file, "rb");
    if (!fp) {
        fprintf (stderr, "can't open `%s': %s\n", md5_file, strerror (errno));
        return 1;
    }
    int retval = 0;
    char sum[33], file[128];
    char ck_sum[33];
    bool pass;
    while (fscanf(fp, "%s  %s\n", sum, file) != EOF) {
        pass = false;
        int ret = md5_sum(file, ck_sum, 0);
        if (!ret) {
            if (!strcmp(sum, ck_sum)) pass = true;
        }
        printf("%s: ", file);
        if (pass) {
            printf("%s\n", "OK");
