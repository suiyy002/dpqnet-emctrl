...
void COtherForm::init_component()
{
	...
	plabelset->top = labelset_lu->top + 14;
	plabelset->width = labelset_lu->width;
	plabelset->height = 15;
	plabelset->font.space = 0;
	//plabelset->font.asc_size = 12;
	plabelset->allrefresh = true;
	plabelset->set_txt(0,"电压不平衡:");
	plabelset->set_txt(2,"负序:");
	plabelset->set_txt(4,"零序:");
	plabelset->labels_[0].top = 0;
	plabelset->labels_[0].left = 0;
	k = 134;
	for(int i=1;i<3;i++){
		plabelset->labels_[2*i].top = 0;
		plabelset->labels_[2*i].left = k;
		k += 94;
	}
	plabelset->labels_[1].top = 0;
	plabelset->labels_[1].left = 68;
	k = 166;
	for(int i=1;i<3;i++){
		plabelset->labels_[2*i+1].top = 0;
		plabelset->labels_[2*i+1].left = k;
		k += 94;
	}
	//电流不平衡度数据初始化
	...
	*plabelset = *labelset_imbalanceu;
	...
	//基波电压矢量图初始化
	vectorchart_u = new CChartVector(3,40);
	pvctrchart = vectorchart_u;
	pvctrchart->left = left+8;
///[PLT
	pvctrchart->top = labelset_imbalancei->top + labelset_imbalancei->height + 2;
///]
...
///[PLT
	...
	plabel->top = labelset_imbalanceu->top-2;
///]
	...
	*plabel = *label_lack_phsu;
///[PLT
	plabel->top = labelset_imbalancei->top-2;
	//plabel->left = left + width/2 + 8;
///]
...
///]
	...
	plabelset->allrefresh = true;
	plabelset->labels_[0].top = 0;
	plabelset->labels_[0].self_font = true;
	plabelset->labels_[0].font.asc_size = 12;
	plabelset->labels_[0].font.space = 1;
...
//	labelset_Plt->set_visible(false);
	labelset_Plt = new CLabelSet(4);
...
void COtherForm::set_view_num(int num, int num2)
{
	...
	if(view_num==1){
		labelset_debug->font.space = 0;
		labelset_debug->allrefresh = false;
	}else if(view_num==2){
		labelset_debug->font.space = -1;
