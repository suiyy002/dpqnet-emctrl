...
void CHarmForm::init_component()
{
	...
	//谐波界面选项标签类初始化
	tabctrl_harm = new CTabControl(HARM_VIEW_TOTL);
	tabctrl_harm->top = 16;
	tabctrl_harm->left = 15;
	...
	chart_harm->axis_x.ticks_spc = (1000*(w+intv)*5)/cht_wd; //刻度间距
	...
	for(int i=0;i<3;i++){
		...
		//plabelset->labels_[2*i].left = 0;
		k += 13;
		plabelset->labels_[2*i+1].top = k;
		//plabelset->labels_[2*i+1].left = 0;
		k +=16;
	}
	...
	plabelset->width = 56;
	plabelset->height = 160;
	plabelset->left = left + width - plabelset->width;
	plabelset->top = top + 24;
	plabelset->allrefresh = true;
	//plabelset->transparent = false;
	//plabelset->bgcolor = vgacolor(kVGA_Default);
	plabelset->font.color = vgacolor(kVGA_Default);
	...
	labelset_harm->width = 207;
	labelset_harm->height = 156;
	...
	labelset_harm_title->left = labelset_harm->left + 58;
	labelset_harm_title->top = labelset_harm->top - 16;
	labelset_harm_title->font.space = 1;
}
